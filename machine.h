...
typedef struct machine {
  ...
  // sfp_port[0] is the first SFP-port from the left on the device, sfp_port[1]
  // the next if present
  struct sfp_port sfp_port[2];
  uint8_t reset_pin;
  struct high_leds high_leds;
  // Defines which led-set (0-3) will be used for given logical port
  // led-set is physical group of LEDs that can be configured to show different
  // port status combinations (see port_led_set below)
  uint8_t port_led_set[9];
  // Defines led-set configuration, applied to all ports using particular
  // led-set Each led-set can have 4 different hardware LED configurations.
  // Which one should be used, depends how LED is wired on the board See stock
  // RTL837X_REG_LED3_2_SETx and RTL837X_REG_LED1_0_SETx registers for reference
  // configuration
